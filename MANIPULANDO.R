...
# Transforma?ao das bases de dados ----------------------------------------

# Transformando a base de dados de DENGUE

# Separando o c?digo e o nome do município em 2 colunas
...
# Modificando o conteúdo das colunas, com as funçÃO mutate() e str_replace_all()
...
# fun?ao as.numeric() para num?rico
...
# Juntando as tabelas para o cálculo da incidencia por 100.000 habitantes
...
# Opção quando as vari?veis nao possuem o mesmo nome em tabelas diferentes

incidencia1_dengue_pr <- left_join(casos_dengue_parana,
                                  populacao_parana,
...
incidencia_dengue_pr <- incidencia_dengue_pr |> # base de dados e pipe
  # fun?ao round() para arredondar em 2 casas decimais
  mutate(taxa_incidencia = round((total/pop_total)*100000, 2),
         # fun?ao if_else para definir alerta em taxa >= a 300/100khab       
...
saveWorkbook(wb, file = "Resumo1_Dengue_PR_2024.xlsx")
